...
public class DashboardService {
	...
	public DashboardService(CurrentUserService currentUserService, UserRepository userRepository,
	...
	public Map<String, Object> currentDashboard() {
		...
		case VISITOR -> throw new ForbiddenException("Visitor cannot access dashboard");
		};
	}
	...
	private Map<String, Object> adminDashboard() {
		...
		result.put("pendingUsers", userRepository.countByStatus(UserStatus.PENDING));
		result.put("totalCourses", courseRepository.count());
		result.put("pendingCourses", courseRepository.countByStatus(CourseStatus.PENDING_APPROVAL));
		result.put("successfulPayments", paymentRepository.countByStatus(PaymentStatus.SUCCESS));

		BigDecimal revenue = paymentRepository.findAll().stream()
				.filter(payment -> payment.getStatus() == PaymentStatus.SUCCESS).map(payment -> payment.getAmount())
	...
	private Map<String, Object> instructorDashboard(User user) {
		...
		result.put("courses", courseRepository.countByInstructorId(user.getId()));
		result.put("enrollments", enrollmentRepository.countByCourseInstructorId(user.getId()));
		return result;
	}
	...
	private Map<String, Object> studentDashboard(User user) {
		...
		result.put("enrollments", enrollmentRepository.countByStudentId(user.getId()));
		result.put("certificates", certificateRepository.countByStudentId(user.getId()));
		return result;
	}
}
